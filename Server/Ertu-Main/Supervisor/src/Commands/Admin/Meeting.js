...
module.exports = {
    ...
    switch (value) {
        case "toplantibaslat":
            let voiceuser = message.guild.members.cache.filter(member => member.roles.highest.position >= ertum.ConfirmerRoles[0].position && member.voice.channel && !member.user.bot)
            let nvoiceuser = message.guild.members.cache.filter(member => member.roles.highest.position >= ertum.ConfirmerRoles[0].position && !member.voice.channel && !member.user.bot)
    ...
    break;
    case "toplantiduyuru":
        let nnvoiceuser = interaction.guild.members.cache.filter(member => member.roles.highest.position >= ertum.ConfirmerRoles[0].position && !member.voice.channel && !member.user.bot)

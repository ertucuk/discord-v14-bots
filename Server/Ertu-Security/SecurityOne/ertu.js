...
if (ertucuk.Security.UrlSystem) {
const urlClient = new VanityClient(ertucuk.Security.SelfBotToken,ertucuk.ServerID,true);
urlClient.on("VanitySuccess", async (response) => { console.log(`${response.vanityURL} URL'si Başarıyla Alındı`) })
urlClient.on('VanityError', async (error) => { console.log(`URL Alınırken Bir Hata Meydana Geldi!!\nHata; ${error.error}`); })
}
...
if (ertucuk.Security.UrlSystem) { urlClient.setVanityURL(ertucuk.ServerURL) }
newGuild.channels.cache.find(x => x.name == "guard_log").send({ content: "@here"});
...
client.on(Events.GuildUpdate, async (oldGuild, newGuild) => {
    ...
    const member = ertu.entries.first();
    if(!(await getGuardStatus("guildGuard"))) return createEmbed(`${member.executor} üyesi sunucuda değişiklik yaptı. Sunucu Guard kapalı olduğu için işlem yapmadım.`)

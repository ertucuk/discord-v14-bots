    ...
    onCommand: async function (client, message, args) {
      ...
      collector.on("collect", async (button) => {
        ...
        if (button.customId === "mute") {
          ...
          if (ertucuk.Moderation.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından susturmanız kaldırıldı!`}).catch(() => {});
      ...
      .setFooter({ text:`${moment(Date.now()).format("LLL")}` })
       await logChannel.send({ embeds: [log1]});
        ...
        if (button.customId === "vmute") {
          ...
          if (ertucuk.Moderation.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından **sesli kanallarda** olan susturmanız kaldırıldı!`}).catch(() => {});
